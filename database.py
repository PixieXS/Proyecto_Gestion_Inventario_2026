 ...
 # ── gestion de roles y permisos
   ...
   # movimientos de los productos
    ...
    def registrar_movimiento(self, id_producto, tipo_movimiento, cantidad,
        ...
        try:
            if tipo_movimiento.lower() == "salida":
                self.cursor.execute(
                    "SELECT cantidad FROM productos WHERE id=%s", (id_producto,))
                row = self.cursor.fetchone()
                if not row:
            ...
            self.cursor.execute("""
                INSERT INTO movimientos (id_producto,tipo_movimiento,cantidad,descripcion,id_usuario)
                VALUES (%s,%s,%s,%s,%s)
            ...
            if tipo_movimiento.lower() == "entrada":
                ...
                sql = "UPDATE productos SET cantidad=cantidad-%s WHERE id=%s"
            self.cursor.execute(sql, (cantidad, id_producto))
            self.connection.commit()
    ...
    def obtener_historial_producto(self, id_producto):
        """Movimientos de un producto con nombre de usuario."""
        try:
            self.cursor.execute("""
                SELECT m.id, m.tipo_movimiento, m.cantidad, m.fecha,
                       m.descripcion, COALESCE(u.username,'Sistema') AS usuario
                FROM movimientos m
                LEFT JOIN usuarios u ON m.id_usuario=u.id
    ...
    def obtener_estadisticas(self):
        try:
            self.ping_and_commit()
            stats = {}
            self.cursor.execute("SELECT COUNT(*) AS n FROM productos")
            stats['total_productos'] = self.cursor.fetchone()['n']
            self.cursor.execute("SELECT SUM(cantidad) AS s FROM productos")
            r = self.cursor.fetchone(); stats['stock_total'] = r['s'] or 0
            self.cursor.execute(
                "SELECT SUM(cantidad*precio_unitario) AS v FROM productos")
            r = self.cursor.fetchone()
            stats['valor_total'] = float(r['v']) if r['v'] else 0.0
            self.cursor.execute(
                "SELECT COUNT(*) AS n FROM productos WHERE cantidad<=stock_minimo")
